...
class BaseStore:
    ...
    async def has_nonce(self, identity_id: str, nonce: str) -> bool: ...
    async def store_nonce(self, identity_id: str, nonce: str): ...
    async def store_otp(self, challenge_id: str, code: str, ttl_seconds: int = 120): ...
...
class InMemoryStore(BaseStore):
    ...
    async def record_request(self, identity_id: str, endpoint: str, geo: str, device: str, hour: int, ts: float, payload_size: int = 0):
        async with self._lock:
            ...
            ps = p.setdefault("payload_sizes", [])
            ps.append(payload_size)
            if len(ps) > MAX_HISTORY_PER_IDENTITY:
                ps.pop(0)
            p["total"] += 1
    ...
    async def has_nonce(self, identity_id: str, nonce: str) -> bool:
        return nonce in self._nonces[identity_id]

    async def store_nonce(self, identity_id: str, nonce: str):
        self._nonces[identity_id].add(nonce)

    async def store_otp(self, challenge_id: str, code: str, ttl_seconds: int = 120):
        self._otps[challenge_id] = {
            "code": code,
    ...
    async def verify_otp(self, challenge_id: str, code: str) -> str:
        ...
        async with self._lock:
            entry = self._otps.get(challenge_id)
            if not entry:
                return "wrong"
            if entry["used"]:
                return "used"
...
class RedisStore(BaseStore):
    ...
    async def record_request(self, identity_id: str, endpoint: str, geo: str, device: str, hour: int, ts: float):
        ...
        # Fetch current profile, mutate, write back — use pipeline so GET+SET
        # are batched in a single round-trip when Redis 6.0+ is available.
    ...
    async def has_nonce(self, identity_id: str, nonce: str) -> bool:
        return bool(await self.r.sismember(f"sentinelx:nonces:{identity_id}", nonce))

    async def store_nonce(self, identity_id: str, nonce: str):
        key = f"sentinelx:nonces:{identity_id}"
        await self.r.sadd(key, nonce)
        await self.r.expire(key, 3600)

    async def store_otp(self, challenge_id: str, code: str, ttl_seconds: int = 120):
        import json as _json
        await self.r.set(
            f"sentinelx:otp:{challenge_id}",
            _json.dumps({"code": code, "used": False}),
            ex=ttl_seconds,
        )

    async def verify_otp(self, challenge_id: str, code: str) -> str:
        import json as _json
        key = f"sentinelx:otp:{challenge_id}"
        raw = await self.r.get(key)
        if not raw:
            return "expired"  # TTL expired or never existed
