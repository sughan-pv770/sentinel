...
function tierColor(tier) {
  return { allow: "#06b6d4", step_up: "#f59e0b", restrict: "#f97316", revoke: "#ef4444" }[tier] || "#f8fafc";
}
...
function updateRiskRing(score) {
  ...
  const currentScore = parseInt(num.textContent) || 0;
  ...
  if (currentScore !== targetScore) {
    ...
    const animateCount = (now) => {
      ...
      const current = Math.round(currentScore + (targetScore - currentScore) * ease);
...
async function pollStats() {
  try {
    ...
    const sd = $("#status-dot");
    if (sd) sd.style.background = "#06b6d4";
...
function updatePipelineUI() {
  ...
  if (s.features && Array.isArray(s.features)) {
    ...
    keys.forEach((key, i) => {
      ...
      if (fb) {
        fb.style.height = `${pct}%`;
        fb.style.background = pct > 70 ? '#ef4444' : pct > 35 ? '#f59e0b' : '#06b6d4';
      }
    });
  }
}
...
if (demoBtn) {
  demoBtn.addEventListener("click", async () => {
    ...
    const scenarios = [
      { method: "GET", endpoint: "/profile", geo: "IN", speed: "normal" },
      { method: "POST", endpoint: "/payments/transfer", geo: "IN", speed: "normal" },
      { method: "GET", endpoint: "/admin/users", geo: "IN", speed: "normal" },
      { method: "GET", endpoint: "/orders", geo: "IN", speed: "spike" },
      { method: "GET", endpoint: "/profile", geo: "RU", speed: "normal" }
    ];

    for (const sc of scenarios) {
      if ($("#sandbox-method")) $("#sandbox-method").value = sc.method;
      if ($("#sandbox-endpoint")) $("#sandbox-endpoint").value = sc.endpoint;
      if ($("#sandbox-geo")) $("#sandbox-geo").value = sc.geo;
      if ($("#sandbox-speed")) $("#sandbox-speed").value = sc.speed;
      
      const sendBtn = $("#sandbox-send");
      if (sendBtn) sendBtn.click();
      
      await new Promise(r => setTimeout(r, 4000)); // wait for scoring and pipeline animation
    }
...
function renderTicker(alerts) {
  ...
  if (!scroll) return;
  // Filter for relevant alerts, excluding 'allow' tier from the ticker
  const relevant = alerts.filter(a => a.identity_id === currentIdentity && a.tier !== 'allow');
  ...
  relevant.slice().reverse().forEach(a => {
    ...
    div.innerHTML = `
      ...
      <div class="explainability-panel" style="display:none; margin-top: 10px; border-top: 1px solid rgba(255,255,255,0.1); padding-top: 10px;">
        <div style="font-size: 10px; color: var(--text-muted); margin-bottom: 8px;">ML FEATURE CONTRIBUTIONS</div>
        ${a.contributions ? ['FREQ', 'NEW-EP', 'GEO', 'DEV', 'TIME', 'PAYLOAD', 'AGE'].map((lbl, i) => `
          <div style="display:flex; align-items:center; gap:8px; margin-bottom:4px; font-size:10px;">
            <div style="width:40px; text-align:right; font-family:var(--font-mono); color:var(--text-muted)">${lbl}</div>
            <div style="flex:1; background:rgba(0,0,0,0.3); height:4px; border-radius:2px; overflow:hidden;">
              <div style="height:100%; width:${a.contributions[i]}%; background:var(--text-accent);"></div>
            </div>
            <div style="width:30px; text-align:right; font-family:var(--font-mono);">${a.contributions[i]}%</div>
          </div>
        `).join("") : '<div style="font-size:10px; color:var(--text-muted)">No contribution data</div>'}
      </div>
    `;
    
    div.style.cursor = "pointer";
    div.addEventListener("click", () => {
      const panel = div.querySelector(".explainability-panel");
      if (panel) {
