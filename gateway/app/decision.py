...
async def decide(ctx: RequestContext, fv: FeatureVector, store: BaseStore) -> RiskDecision:
    ...
    ml_score = ml_engine.score(fv)
    model_version = getattr(ml_engine, "model_version", "unknown")
    ...
    endpoint_risk_score, endpoint_sensitivity = calculate_endpoint_risk(ctx)
    ...
    # Approximate feature contributions using z-score distance from baseline
    means = [3.0, 0.0, 0.0, 0.0, 0.15, 0.0, 1800.0]
    stds = [2.0, 0.1, 0.1, 0.1, 0.15, 1.0, 900.0]
    raw_contribs = [abs(val - m) / s for val, m, s in zip(feature_arr, means, stds)]
    total_contrib = sum(raw_contribs) if sum(raw_contribs) > 0 else 1.0
    contributions = [round((c / total_contrib) * 100, 1) for c in raw_contribs]
