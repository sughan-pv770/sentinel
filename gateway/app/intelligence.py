...
def calculate_endpoint_risk(endpoint: str) -> Tuple[float, str]:
    """
    ...
    # Critical endpoints
    if any(prefix in endpoint_lower for prefix in ["/admin", "/internal", "/system", "/config"]):
        return 90.0, "CRITICAL"

    # High-risk endpoints
    if any(prefix in endpoint_lower for prefix in ["/payments", "/billing", "/transfer", "/delete"]):
        return 75.0, "HIGH"

    # Moderate-risk endpoints
    if any(prefix in endpoint_lower for prefix in ["/users", "/accounts", "/profile", "/settings"]):
